...
export const getTransactions = async (userId, limit = 500) => {
  ...
  const response = await fetch(
    `${API_BASE_URL}/transactions/transactions?user_id=${encodeURIComponent(userId)}&limit=${encodeURIComponent(limit)}`
  );
...
export const getTransactionsByBuyer = async (userId, buyerId, limit = 500) => {
  ...
  const response = await fetch(
    `${API_BASE_URL}/transactions/transactions?user_id=${encodeURIComponent(userId)}&buyer_id=${encodeURIComponent(buyerId)}&limit=${encodeURIComponent(limit)}`
  );
  const data = await parseResponseBody(response);
  if (!response.ok) {
...
export const updateTransaction = async ({ userId, transactionId, amount, transactionName = null }) => {
  ...
  const response = await fetch(
    `${API_BASE_URL}/transactions/transactions/${encodeURIComponent(transactionId)}`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
